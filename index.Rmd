...
g2 <- ggplot(data = e_df, aes(x=MEAN)) 
...
g2 +stat_function(fun = dnorm, colour = "magenta", args = list(mean = t_mean, sd = sqrt(t_var)))
```

```{r}
e_df %>% ggplot(aes(sample=MEAN))+stat_qq()+stat_qq_line(color="blue")+labs(x="Theoretical Quantile", y="Sample Quantiles", title = "Quantile - Quantile Plot")
```
